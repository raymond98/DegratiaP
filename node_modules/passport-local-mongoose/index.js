...
module.exports = function(schema, options) {
  ...
  options.errorMessages.MissingUsernameError = options.errorMessages.MissingUsernameError || 'No email was given';
  ...
  schema.statics.authenticate = function() {
    return (email, password, cb) => {
      const promise = Promise.resolve()
      .then(() => this.findByUsername(email, true))
      .then((user) => {
        if (user) {
  ...
  schema.statics.findByUsername = function(email, opts, cb) {
    ...
    if (email !== undefined && options.usernameLowerCase) {
    ...
    for (let i = 0; i < options.usernameQueryFields.length; i++) {
      const parameter = {};
      parameter[options.usernameQueryFields[i]] = options.usernameCaseInsensitive ? new RegExp(`^${email}$`, 'i') : email;
